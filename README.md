...
- **Шаблоны:** Handlebars
- **Стили:** SCSS
- **Деплой:** [GitHub Pages](https://maximste.github.io/GilgaChat/) — основной публичный билд. **Netlify:** конфигурация в `netlify.toml`; URL и статус сборки в кабинете Netlify.
...
Используется **Jest**; файлы `*.test.ts` лежат рядом с модулями в `src/`.
...
Проверка типов и линтеров: `npm run prebuild`, `npm run lint`.
